...
begin
  ...
  insert into public.integrations(code,integration_year,sequence_number,description,client,responsible,family,subfamily,subsubfamily,notes,created_by)
  values(c,y,n,p_data->>'description',p_data->>'client',p_data->>'responsible',nullif(p_data->>'family',''),nullif(p_data->>'subfamily',''),nullif(p_data->>'subsubfamily',''),nullif(p_data->>'notes',''),auth.uid())
  returning * into rec;
